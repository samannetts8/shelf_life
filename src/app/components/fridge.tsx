  ...
  return (
    <div className="space-y-6">
      {expiringSoon.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-red-600 mb-2 flex items-center">
          ...
          <div className="space-y-2">
            {expiringSoon.map((item) => (
              <FoodItem key={item.id} item={item} onConsume={markAsConsumed} status="expiring" />
            ))}
          </div>
        </section>
      )}
      ...
      {goodItems.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-green-600 mb-2 flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" viewBox="0 0 20 20" fill="currentColor">
              <path
                fillRule="evenodd"
          ...
          <div className="space-y-2">
            {goodItems.map((item) => (
              <FoodItem key={item.id} item={item} onConsume={markAsConsumed} status="good" />
            ))}
          </div>
        </section>
      )}
      ...
      {expiredItems.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-gray-600 mb-2 flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" viewBox="0 0 20 20" fill="currentColor">
              <path
                fillRule="evenodd"
          ...
          <div className="space-y-2">
            {expiredItems.map((item) => (
              <FoodItem key={item.id} item={item} onConsume={markAsConsumed} status="expired" />
            ))}
          </div>
        </section>
      )}
